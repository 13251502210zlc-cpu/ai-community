...
- 主版本：存在不兼容的接口路径、请求或响应变更；
- 次版本：新增接口或新增兼容路径；
- 修订号：仅修正文案、示例、字段说明或错误描述，不改变接口行为。
...
以后更新本接口文档时必须同时完成以下事项：

1. 修改文档版本号和更新日期；
2. 在修订记录表顶部新增一行，保留全部历史记录，不得覆盖或删除旧记录；
3. 记录新增、修改、废弃或删除的接口路径，以及请求参数、响应结构、鉴权和错误码变化；
4. 接口废弃时先标记 `Deprecated` 并注明替代接口和计划移除版本；
5. 重新核对源码路由数量、示例地址及兼容路径后再发布文档。
